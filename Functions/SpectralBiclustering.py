...
class Spectral_Biclustering:
    ...
    dataframe then tells you which column clusters are associatted with each true group. The column clusters contain a set
    ...
    def __init__(self, data_func, data_frame = False):
        ...
        ----------
        data_func : function
            Should load the data, in the form x = pandas df of data, y = array of groups, X_pca = not used.
    ...
    def perform_cluster(self, n_row_clusters = 6, n_col_clusters = 9):
        ...
        X_scaled = scale(X_log, axis=0)          # Pareto or z-scaling per metabolite (column-wise)
        ...
        # Extract results
        row_labels = model.row_labels_       # cluster assignment per sample (length 30)
        col_labels = model.column_labels_    # cluster assignment per metabolite (length 600)
    ...
    def show_heatmap(self):
        ...
        is generated by seaborn, and is not the same clustering as done by the algorithm, although it will be
    ...
    def heatmap_groupings(self, n_clusters_visual = 9, show = True, output = False):
        ...
        # Cut the dendrogram into, say, 9 clusters (pick a number that matches what you see visually)
        n_clusters_visual = n_clusters_visual
        visual_col_clusters = fcluster(col_linkage, t=n_clusters_visual, criterion='maxclust')

        # Map back to metabolite names — note: clustermap reorders columns,
    ...
    def compare_silhouettes(self):
        ...
        Allows motivation of clustering peramature choices.
    ...
    def stats_tests(self, group_a = "AdSKO-hB2", group_b = "SKO-PBS"):
        """
